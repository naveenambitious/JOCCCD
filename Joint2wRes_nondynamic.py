...
CACHING_ENABLED = True  # Set from sys.argv[2] in main(); True = original caching model, False = no-caching baseline
...
class BaseStation(object):
    ...
    def execute_task(self, task):
        ...
        global cacheHits, cacheMiss, Record

        if CACHING_ENABLED and int(self.name.split('_')[1]) == int(cache_location_BS.name.split('_')[1]):
            # ---- CACHE HIT: data already local, execute immediately ----
            ...
            # ---- CACHE MISS: migrate task to where the data lives, then it will hit next time ----
            ...
            # ---- NO-CACHING BASELINE: data is never kept local, so every task pays the
            # full data-fetch cost from its origin base station on every execution, then
            # runs right here (no migration/requeue hop). ----
            cacheMiss = cacheMiss + 1
            Data_fetch_time = ((self.distance_BSs(cache_location_BS.location, self.location)) / (2.4 * 10**8)) + (task.task_size / (10**6))
            total_data_migration_time.append(Data_fetch_time)
            yield self.env.timeout(Data_fetch_time)
            yield self.env.timeout( (task.task_size/self.instruction_per_cyle) * (self.clock_cycle_time))  # Execution time
            currentTime = self.env.now
            Record['Dev_id'] = Record['Dev_id'].astype(int)
            Record['Job_id'] = Record['Job_id'].astype(int)
            self.RCD.loc[len(self.RCD)] = {'Dev_id': int(task.name_prefix.split('_')[1]), 'Job_id': int(task.task_counter), 'Execution_time': currentTime, 'waiting_time': WT,'task_status': 'Completed'}
...
class RelayAgent(object):
    ...
    def remove_task(self, queue_type, BSstations):
        ...
        ##-trans_time = (1024 * 1000) / ((50.0001 * math.log2(1 + 45.86)) * 1000)
...
class GenTask(object):
    ...
    def generator_task(self):
        while True:
                   ...
                   #global Record
                   self.record.loc[len(self.record)] = {'Dev_id': int(self.name_prefix.split('_')[1]), 'Job_id': int(self.task_counter), 'Arraival_time': self.env.now, 'Execution_time': self.prio, 'Status':'Not Completed'}
...
def main():
    ...
    # Optional 2nd CLI arg toggles the no-caching baseline: python Joint2wRes.py <dev_num> nocache
    global CACHING_ENABLED
    if len(sys.argv) > 2 and sys.argv[2].strip().lower() in ('nocache', 'no-cache', 'no_cache', '0', 'false'):
        CACHING_ENABLED = False
    else:
        CACHING_ENABLED = True
    mode_tag = 'cache' if CACHING_ENABLED else 'nocache'
    print(f"Running with CACHING_ENABLED = {CACHING_ENABLED}")
    ...
    plot_entities(base_stations, relay_agents, devices, mode_tag+'_before')
    env.run(until=600)
    ...
    merged_df.to_csv('out'+str(dev_num)+'_'+mode_tag+'.csv', index=False)
    #print(merged_df)
    #print(merged_df)
    ...
    #plt.show()
    plot_entities(base_stations, relay_agents, devices, mode_tag+'_after')
    ...
    #plt.show()
    results=[]
    results.append([dev_num,CACHING_ENABLED,task_completion_rate,cache_hit_ratio,average_response_time,QoE_Score,avgDataMig/len(total_data_migration_time),average_execution_time['Execution_time'].mean()])
    results_df = pd.DataFrame(results, columns=[ 'device_no', 'caching_enabled', 'task_completion_rate','cache_hit_ratio','average_response_time','QoS_score','Datamig_time','exec_time_mean'])
    import os
    write_header = not os.path.exists('simulation_results.csv')
    results_df.to_csv('simulation_results.csv', index=False, mode='a', header=write_header)
